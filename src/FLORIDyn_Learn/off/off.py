...
# Go up 3 levels: off/__init__.py -> off/ -> 03_Code/ -> OFF root
# Using os.path for Windows compatibility (original used rsplit('/', 3) which fails on Windows)
OFF_PATH = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(OFF_PATH))))
...
class OFF:
    ...
    def __dir_init__(self, settings_sim: dict):
        ...
        try:
            root_dir = data_dir or os.path.join(os.environ["OFF_PATH"], 'runs')
            lg.info('Root runs directory: ' + root_dir)
        except KeyError:
            # Fallback: put runs/ next to the off package
            root_dir = data_dir or os.path.join(OFF_PATH, 'runs')
            lg.warning('Initial root runs directory path retrieval was unsuccessful, used ' + root_dir)

        self.sim_dir = os.path.join(root_dir, f'off_run_{run_id}') if sim_dir is None else sim_dir
