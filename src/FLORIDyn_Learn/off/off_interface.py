...
class OFFInterface:
    ...
    def init_simulation_by_path(self, path_to_yaml: str):
        ...
        tmp_yaml_path = self._gen_FLORIS_yaml(settings_wke,
                                              sim_info["wind_farm"],
                                              sim_info["ambient"],
                                              os.path.dirname(os.path.abspath(path_to_yaml)))
    ...
    def store_measurements(self, path_to_csv=""):
        ...
        if len(path_to_csv) == 0:
            path_to_csv = os.path.join(self.off_sim.sim_dir, "measurements.csv")
    ...
    def store_run_file(self):
        ...
        shutil.copyfile(self.off_sim.settings_sim['path_to_yaml'],
                        os.path.join(self.off_sim.sim_dir, os.path.basename(self.off_sim.settings_sim['path_to_yaml'])))
    ...
    def store_applied_control(self, path_to_csv=""):
        ...
        """
        if len(path_to_csv) == 0:
            path_to_csv = os.path.join(self.off_sim.sim_dir, "applied_control.csv")
    ...
    def _gen_FLORIS_yaml(self, settings_wke: dict, settings_wf: dict, settings_amb: dict, path_to_tmp: str) -> str:
        ...
        stream = open(os.path.join(off.OFF_PATH, settings_wke["floris_logging"].replace('/', os.sep)), 'r')
        f_logging = yaml.safe_load(stream)
        floris_file.update(f_logging)

        stream = open(os.path.join(off.OFF_PATH, settings_wke["floris_solver"].replace('/', os.sep)), 'r')
        f_solver = yaml.safe_load(stream)
        floris_file.update(f_solver)

        stream = open(os.path.join(off.OFF_PATH, settings_wke["floris_wake"].replace('/', os.sep)), 'r')
        ...
        path_out = os.path.join(path_to_tmp, 'tmp_floris_input' + current_time.strftime("%Y%m%d%H%M%S%f") + '.yaml')
